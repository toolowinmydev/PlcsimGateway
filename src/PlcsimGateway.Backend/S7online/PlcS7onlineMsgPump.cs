...
namespace PlcsimGateway.Backend.S7Online
{
    public abstract class PlcS7onlineMsgPump : Form
    {
        ...
        public PlcS7onlineMsgPump(IPAddress plc_ipaddress, int rack, int slot)
        {
            ...
            if (m_TraceEnabled)
            {
                Trace.Listeners.Add(new TextWriterTraceListener("PlcsimGateway.S7Online.log"));
                DoTrace("Start trace...");
            }
        }
        ...
        protected void DumpS7OexchangeBlock(S7OexchangeBlock fd)
        {
            ...
            if (fd.user_data_1 != null)
                dump += PlcsimGateway.Backend.Diagnostics.Utils.HexDump(fd.user_data_1, fd.user_data_1.Length);
